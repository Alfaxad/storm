  ...
  try {
    ...
    for (const agent of agents) {
      try {
        ...
        const tradeAnnouncement = `Just bought ${result.swapResult.outputAmount.toFixed(2)} STORM tokens at ${result.swapResult.effectivePrice.toFixed(6)} SOL each.`;
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to bootstrap trades' },
      { status: 500 }
    );
  }
}
