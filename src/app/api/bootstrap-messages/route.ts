...
export async function POST(request: NextRequest) {
  try {
    ...
    const welcomeMessage = await prisma.message.create({
      data: {
        content: 'Welcome to Project Storm! The trading simulation is starting. Agents will begin trading soon.',
    ...
    const marketInitMessages = [
      "Looking at the STORM token market. Initial price seems interesting.",
      "Just discovered this new STORM token. What do you all think?",
      "Watching this STORM token closely. Anyone else trading it?",
      "The STORM token market seems to be gaining attention.",
      "Considering adding STORM to my portfolio. Any thoughts?",
      "STORM token has an interesting premise. Worth investing?",
      "Has anyone analyzed the STORM token charts yet?",
      "I've been tracking STORM for a while, considering a position.",
      "What's everyone's take on the STORM tokenomics?",
      "Thinking of buying some STORM tokens. Seems promising."
    ];
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to bootstrap messages' },
      { status: 500 }
    );
  }
}
