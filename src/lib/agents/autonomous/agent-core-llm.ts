...
class TokenSwapTool extends Tool {
  ...
  description = "Execute a token swap between SOL and the STORM token. Input should be a JSON object with parameters: inputAmount (number), inputIsSol (boolean), and optionally slippageTolerance (number).";
  ...
  async _call(input: string) {
    try {
      ...
      try {
        ...
        if (!args) {
          return JSON.stringify({ 
            ...
            details: "Please provide input as JSON or a clear statement like 'buy 5 SOL worth of STORM'" 
          });
        }
      }
      ...
      console.log(`[TRADE] Agent ${this.agentId} attempting to ${inputIsSol ? 'buy' : 'sell'} with ${inputAmount} ${inputIsSol ? 'SOL' : 'STORM'}`);
  ...
  private parseTextInput(text: string): { inputAmount: number, inputIsSol: boolean } | null {
    ...
    // Buy pattern (SOL -> STORM)
    const buyPattern = /buy\s+(\d+(\.\d+)?)\s*(sol\s+worth\s+of\s+)?storm|purchase\s+(\d+(\.\d+)?)\s*(sol\s+worth\s+of\s+)?storm|acquire\s+(\d+(\.\d+)?)\s*(sol\s+worth\s+of\s+)?storm/i;

    // Sell pattern (STORM -> SOL)
    const sellPattern = /sell\s+(\d+(\.\d+)?)\s*storm|convert\s+(\d+(\.\d+)?)\s*storm\s+to\s+sol|exchange\s+(\d+(\.\d+)?)\s*storm\s+for\s+sol/i;
    ...
    if (match) {
      ...
      return {
        inputAmount: amount,
        inputIsSol: true // Buying STORM with SOL
      };
    }
    ...
    if (match) {
      ...
      return {
        inputAmount: amount,
        inputIsSol: false // Selling STORM for SOL
      };
    }

    // Try simpler patterns if the above didn't match
    if (text.includes('buy') && text.includes('storm')) {
      return {
        inputAmount: 5,
        inputIsSol: true
      };
    ...
    if (text.includes('sell') && text.includes('storm')) {
      return {
        inputAmount: 5,
        inputIsSol: false
      };
  ...
  private async initializeAgent() {
    try {
      ...
      const systemPrompt = new SystemMessage(
        ...
        `${this.personalityPrompt}\n\n` +
        `## Your Role\n` +
        `As a ${agentContext.personality} trader in the STORM token market, your goal is to maximize profits ` +
        ...
        `- Make trading decisions (buy/sell STORM tokens with SOL)\n` +
        ...
        `- STORM Balance: ${agentContext.balance.token} STORM\n` +
  ...
  async analyzeMarket(marketInfo: any) {
    try {
      ...
      const marketSummary =
        ...
        `- STORM token price: ${marketInfo.price} SOL\n` +
  ...
  async makeTradeDecision(marketInfo: any) {
    try {
      ...
      const balanceInfo =
        ...
        `- STORM tokens: ${agent.tokenBalance} STORM\n\n`;
      ...
      const response = await this.agent.invoke({
        messages: [
          new HumanMessage(
            ...
            `1. Buy STORM tokens with SOL\n` +
            `2. Sell STORM tokens for SOL\n` +
