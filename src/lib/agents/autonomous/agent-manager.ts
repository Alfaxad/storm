...
export class AgentManager {
  ...
  public async processAgentsForTrading(batchSize: number = 50): Promise<void> {
    ...
    if (tradesToExecute.length > 0) {
      await pMap(
        ...
        async (trade) => {
          try {
            console.log(`Executing trade for agent ${trade.agentId}: ${trade.action} ${trade.inputAmount} ${trade.inputIsSol ? 'SOL' : 'STORM tokens'}`);
            ...
            try {
              ...
              const tradeAnnouncement = trade.inputIsSol ?
                `Just bought ${result.swapResult.outputAmount.toFixed(2)} STORM tokens at ${result.swapResult.effectivePrice.toFixed(6)} SOL each.` :
                `Just sold ${trade.inputAmount} STORM tokens for ${result.swapResult.outputAmount.toFixed(4)} SOL.`;
  ...
  public async executeTradeForAgent(agentId: string, inputAmount: number, inputIsSol: boolean): Promise<any> {
    try {
      console.log(`Executing bootstrap trade for agent ${agentId}: ${inputIsSol ? 'buy' : 'sell'} ${inputAmount} ${inputIsSol ? 'SOL' : 'STORM'}`);
