...
export class SimulationEngine {
    ...
    try {
      ...
      if (messageCount === 0) {
        ...
        const welcomeMessage = await prisma.message.create({
          data: {
            content: 'Welcome to Project Storm! The trading simulation is starting.',
          ...
          try {
            ...
            if (agent) {
              ...
              const message = await prisma.message.create({
                data: {
                  content: `I'm looking at the STORM token market. Initial price seems to be set at ${initialPrice} SOL.`,
                  senderId: agentId,
                  type: 'MARKET_UPDATE',
                  visibility: 'public',
        ...
        for (let i = 0; i < 3; i++) {
          ...
          try {
            ...
            await prisma.message.create({
              data: {
                content: `Just bought some STORM tokens. I like the initial price point.`,
                senderId: agentId,
                type: 'TRADE',
                visibility: 'public',
                sentiment: 'positive',
  ...
  /**
   * Get the initial STORM token price
   */
